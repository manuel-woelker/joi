...
export function StatusBar(props: { registry: PluginRegistry }) {
  ...
  return (
    <footer class={styles.statusBar}>
      <StatusBarSide contributions={left} side="left" registry={props.registry} />
      <StatusBarSide contributions={right} side="right" registry={props.registry} />
    </footer>
  );
}

function StatusBarSide(props: {
  contributions: StatusBarContribution[];
  side: "left" | "right";
  ...
  return (
    <div class={`${styles.side} ${styles[props.side]}`}>
      <For each={props.contributions}>
        {(contribution) => <Dynamic component={contribution.content} pluginRegistry={props.registry} />}
      </For>
    </div>
  );
}
