      ...
      <Show when={controller.selectedView()} fallback={<div class="empty-state"><h1>No view selected</h1><p>Create or select a saved view from the navigation.</p></div>}>
        {(view) => <>
          ...
          <Show when={!validation()} fallback={<div class="error-state">{validation()}</div>}>
            <Show when={!ticketRecords.loading} fallback={<div class="empty-state compact"><h2>Loading issues</h2></div>}>
            <Show when={!ticketRecords.error} fallback={<div class="error-state"><p>Tickets could not be loaded.</p><button class="secondary" onClick={() => refetch()}>Retry</button></div>}>
            <Show when={records().length} fallback={<div class="empty-state compact"><h2>No matching issues</h2><p>Adjust the search or view query.</p></div>}>
              <Show when={presentation()?.layout === "table"} fallback={
                <div class="issue-list"><For each={records()}>{(ticket) => <article><div><strong>{ticket.title}</strong><span>{ticket.description}</span></div><div class="issue-meta"><span class={`status ${ticket.status}`}>{ticket.status}</span><span>{ticket.id}</span></div></article>}</For></div>
              }>
