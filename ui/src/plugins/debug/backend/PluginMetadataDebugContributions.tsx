    ...
    <ul class="debug-metadata-list">
      <For each={props.metadata.plugins}>{(plugin) => (
        <li>
          ...
          <dl>
            <dt>Extension points:</dt><dd>{plugin.extension_points.join(", ") || "None"}</dd>
            <dt>Extensions:</dt><dd>{plugin.extensions.join(", ") || "None"}</dd>
          </dl>
        </li>
      )}</For>
    </ul>
  ...
  return (
    <Switch>
      <Match when={props.metadata.error}>
        <p class="debug-error" role="alert">{props.metadata.error.message}</p>
      </Match>
