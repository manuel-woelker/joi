...
export class PluginRegistryBuilder {
  ...
  build(): PluginRegistry {
    ...
    for (const candidate of orderedPlugins) {
      ...
      const stagedIds = new Set(pointIds);
      candidate.registerExtensionPoints?.(
        new ExtensionPointContext(stagedPoints, stagedIds, resolvePluginServices(candidate, serviceValues)),
      );
      const added = [...stagedPoints.values()].filter((point) => !pointIds.has(point.id));
      replaceMap(points, stagedPoints);
      replaceSet(pointIds, stagedIds);
      pluginInfo.push({
        name: candidate.name,
    ...
    for (const [index, candidate] of orderedPlugins.entries()) {
      const staged = cloneExtensions(extensions);
      const stagedIds = new Set(extensionIds);
      candidate.registerExtensions?.(
        new ExtensionContext(points, stagedIds, staged, resolvePluginServices(candidate, serviceValues)),
      );
...
function initializeServices(plugins: UiPlugin[], initial: readonly InitialService<unknown>[]) {
  ...
  for (const candidate of plugins)
    for (const key of Object.values(candidate.provides)) {
      if (values.has(key.token) || providers.has(key.token))
        throw new Error(`Service '${key.id}' has multiple providers`);
      providers.set(key.token, candidate);
    }
  const dependencies = new Map(plugins.map((candidate) => [candidate, new Set<UiPlugin>()]));
  for (const candidate of plugins)
    for (const key of Object.values(candidate.requires)) {
      if (values.has(key.token)) continue;
      const provider = providers.get(key.token);
      if (!provider) throw new Error(`Plugin '${candidate.name}' requires missing service '${key.id}'`);
      dependencies.get(candidate)!.add(provider);
    }
  const ordered: UiPlugin[] = [];
  const remaining = new Set(plugins);
  while (remaining.size) {
    const ready = [...remaining]
      .filter((candidate) => [...dependencies.get(candidate)!].every((dependency) => !remaining.has(dependency)))
      .sort((a, b) => a.name.localeCompare(b.name));
    if (!ready.length)
      throw new Error(
        `Plugin service dependency cycle: ${[...remaining]
          .map((plugin) => plugin.name)
          .sort()
          .join(" -> ")}`,
      );
    for (const candidate of ready) {
      const required = resolve(candidate.requires, values);
      const provided = candidate.initialize(required);
      const expected = Object.keys(candidate.provides).sort();
      const actual = Object.keys(provided).sort();
      if (expected.join("\0") !== actual.join("\0"))
        throw new Error(`Plugin '${candidate.name}' did not create exactly its declared services`);
      for (const alias of expected) {
        if (provided[alias] === undefined)
          throw new Error(
            `Plugin '${candidate.name}' returned undefined for service '${candidate.provides[alias].id}'`,
          );
...
function resolvePluginServices(plugin: UiPlugin, values: Map<symbol, unknown>): AnyServices {
  return { ...resolve(plugin.requires, values), ...resolve(plugin.provides, values) };
}
function resolve(definitions: ServiceDefinitions, values: Map<symbol, unknown>): AnyServices {
