...
tool. A later lifecycle simplification reserves directories named `generated`
exclusively for codegen and removes those directories wholesale before every
...
- Generated output folders are intentionally disposable and must never contain
  handwritten integration code. The enforced `generated` directory name limits
